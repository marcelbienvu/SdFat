    ...
    for (uint8_t nf = 1; nf <= MAX_FILES; nf *= 2) {
      ...
      for (uint32_t ir = 0; ir < nr; ir++) {
        for (uint32_t i = 0; i < nf; i++) {
          lat = latCmd = micros();
          if (ns*512 != file[i].read(buf, ns*512, NON_BLOCKING)) error("randomRead");
          latCmd = micros() - latCmd;
          while(sd.isBusyAdma());
          lat = micros() - lat;
          if (latCmd > maxlatCmd) {
            maxlatCmd = latCmd;
  ...
  for (uint8_t ns = MAX_SECTORS; ns > 0; ns /= 2) {
    for (uint8_t nf = 1; nf <= MAX_FILES; nf *= 2) {
      ...
      for (uint32_t ir = 0; ir < nr; ir++) {
        for (uint32_t i = 0; i < nf; i++) {
          lat = latCmd = micros();          
          if (ns*512 != file[i].write(buf, ns*512, DMA)) error("randomWrite");
          file[i].write(buf, ns * 512);
          latCmd = micros() - latCmd;
          while(sd.isBusyAdma());
          lat = micros() - lat;
          if (latCmd > maxLatCmd) {
